...
namespace CleanArchitecture.Application.Sales.Queries.GetHighestItemSalesQuery
{
    public class GetHighestItemSalesQuery : IRequest<string>
    {
        public DateTime TimeRange { get; set; }
    }

    public class GetHighestItemSalesQueryHandler : IRequestHandler<GetHighestItemSalesQuery, string>
    {
        private readonly IApplicationDbContext _context;
        private readonly IMapper _mapper;

        public GetHighestItemSalesQueryHandler(IApplicationDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<string> Handle(GetHighestItemSalesQuery request, CancellationToken cancellationToken)
        {
            var sales = await _context.SalesRecord
                .Where(x => x.SalesDate.Month.Equals(request.TimeRange.Month) && x.SalesDate.Year.Equals(request.TimeRange.Year) )
            ...
            return highestSellingItem.ItemName;
        }
    }
}
